...
var defaultCertPNG = "img/cer01.png";
var defaultFontSize = 5;
var defaultFont = "Pattaya";
...
if (downloadType == "png" || downloadType == "jpg") {
  ...
  // สร้าง PDF โดยให้ขนาดเท่ากับ canvas
  var pdf = new jsPDF({
    orientation: canvasWidth > canvasHeight ? 'landscape' : 'portrait',
    unit: 'px',  // ใช้หน่วยเป็น pixel
   // format: [canvasWidth, canvasHeight]  // กำหนดขนาดเอกสารให้ตรงกับ canvas
  // format: [canvasWidth * 0.5, canvasHeight * 0.5]
  });

  // เพิ่มภาพจาก canvas ลงใน PDF
  pdf.addImage(canvas.toDataURL("image/png"), "PNG", 0, 0, canvasWidth* 0.18, canvasHeight*-0.118);
...
function addField(text = "ชื่อ-ฟิลด์", position = [20, 20], editable = true) {
  var data = `
 <div>
 ...
 <input
   ...
   value="${text}"
   data-fontsize="2.5"
   data-font="'TH-Sarabun-PSK', Pattaya"
   data-textalign="left"
   data-x="${position[0]}"
   data-y="${position[1]}"
   data-color="#000"
...
downloadZipButton.addEventListener("click", function (e) {
  ...
  var zipFilename = "ไฟล์เกียรติบัตร.zip";
  var effectiveDOMs = [];
  var dataIndex = [];
